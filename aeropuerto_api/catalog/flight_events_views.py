...
from .mongo_serializers import FlightEventsSerializer
...
def flight_events_list_create(request):
    ...
    serializer = FlightEventsSerializer(data=request.data)
...
def flight_events_detail(request, id: str):
    ...
    if request.method in ["PUT", "PATCH"]:
        serializer = FlightEventsSerializer(data=request.data, partial=(request.method == "PATCH"))
        serializer.is_valid(raise_exception=True)
